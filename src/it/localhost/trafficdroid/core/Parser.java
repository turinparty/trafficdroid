...
public class Parser {
	...
	public static List<ZoneDTO> parse(int mapId, String url) throws CoreException {
		...
		try {
			divsA = ContentDAO.getContent(mapId, url).getDocumentElement().getElementsByTagName(Const.DIV);
		} catch (DaoException e) {
			throw new CoreException(CoreException.DaoException, e.getMessage());
		}
		for (int i = 0; i < divsA.getLength(); i++) {
			id = divsA.item(i).getAttributes().getNamedItem(Const.ID);
			if (id != null && id.getNodeValue().equalsIgnoreCase(Const.SECTION)) {
				divsB = divsA.item(i).getChildNodes();
				for (int y = 0; y < divsB.getLength(); y++) {
					if (divsB.item(y).getAttributes().getNamedItem(Const.ID).getNodeValue().equalsIgnoreCase(Const.ROADBOX)) {
						...
						for (int z = 0; z < trs.getLength() - 1; z += 2) {
							...
							zone.setCatLeft(Integer.parseInt(trs.item(z + 1).getChildNodes().item(1).getAttributes().getNamedItem(Const.CLASS).getNodeValue().substring(2, 3)));
							zone.setCatRight(Integer.parseInt(trs.item(z + 1).getChildNodes().item(2).getAttributes().getNamedItem(Const.CLASS).getNodeValue().substring(2, 3)));
