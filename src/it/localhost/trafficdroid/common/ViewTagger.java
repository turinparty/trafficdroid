...
/**
 * Using View.setTag(int, Object) causes a memory leak if the tag refers, by a
 * strong reference chain, to the view itself (ie. it uses the 'Holder'
 * pattern). This bug is documented here:
 * 
 * http://code.google.com/p/android/issues/detail?id=18273
 * 
 * It seems that an 'interesting' design choice was made to use the view itself
 * as a weak key to the into another collection, which then causes the views to
 * never be GC'd.
 * 
 * The work-around is to *not* use strong refs, or use setTag(Object). But we
 * use multiple tags.
 * 
 * So this class implements setTag(int, Object) in a non-leaky fashion and is
 * designed to be stored in the tag of a view.
 * 
...
public class ViewTagger {
	 ...
	 *            View with tag
	 * @param autoCreate
	 *            Indicates if tagger should be created if not present
	 * 
	 ...
	 * 
	 * @param v
	 *            View from which to retrieve tag
	 * @return
	 */
	 ...
	 *            View from which to retrieve tag
	 * @param value
	 *            Object to store at specified tag
	 */
